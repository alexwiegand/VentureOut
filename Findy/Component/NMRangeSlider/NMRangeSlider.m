 ...
 -(CGRect) trackBackgroundRect
{
    ...
    trackBackgroundRect.size = CGSizeMake(_trackBackgroundImage.size.width - 23, _trackBackgroundImage.size.height);
    ...
    {
        trackBackgroundRect.size.width=self.bounds.size.width - 23;
    }
    
    trackBackgroundRect.origin = CGPointMake(11.5, (self.bounds.size.height/2.0f) - (trackBackgroundRect.size.height/2.0f));
...
//    NSLog(@"Bouds Width = %f", self.bounds.size.width);
//    NSLog(@"Thumb Rect Width = %f", thumbRect.size.width);
//    NSLog(@"Value = %f", value);
//    NSLog(@"Min Value = %f", _minimumValue);
//    NSLog(@"Max Value = %f", _maximumValue);
//    
    float xValue = ((self.bounds.size.width - thumbRect.size.width)*((value - _minimumValue) / (_maximumValue - _minimumValue)));
    
//    NSLog(@"Total X Value = %f\n----------------------------",xValue);
...
-(BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    if(_lowerHandle.highlighted)
    {
        ...
        NSLog(@"%f --- %f", newValue, _lowerValue);
