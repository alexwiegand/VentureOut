...
- (void)removeImageForKey:(NSString *)urlString {
...
- (void)setImage:(UIImage *)image
{
    ...
    if (image.size.width != image.size.height) {
        float size = (image.size.width > image.size.height) ? image.size.height : image.size.width;
        image = [[ImageResizingUtility instance] imageByCropping:image _targetSize:CGSizeMake(size, size)];
    }
    
    if (bCircle == 1) {
        super.image = (image != nil) ? [(AppDelegate *)[[UIApplication sharedApplication] delegate] maskImage:image withMask:[UIImage imageNamed:@"ProfileButtonMask.png"]] : [UIImage imageNamed:@"EmptyPlaceImage.png"];
    } else if (bCircle == 2) {
        super.image = (image != nil) ? [(AppDelegate *)[[UIApplication sharedApplication] delegate] maskImage:image withMask:[UIImage imageNamed:@"DealsImageMask.png"]] : [UIImage imageNamed:@"EmptyPlaceImage.png"];
    } else {
        super.image = image;
    }
}
